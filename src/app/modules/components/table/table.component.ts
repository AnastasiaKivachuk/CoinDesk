...
export class TableComponent implements OnInit {
  public data: { day: string, value: number}[] = [];

  public size: number = 500;
  @ViewChild('g') el: ElementRef;
  options = {
  ...
  ngOnInit() {
    this.service.takeDates().subscribe((item) => {
      this.data = Object.keys(item.bpi).map(item2 => {
        return {day: item2, value: item.bpi[item2]};
      });
      this.description = {
        show: false,
        name: null,
